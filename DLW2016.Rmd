...
dlw_mean <- data.frame()
dlw_mean <- aggregate(dlw$kJ_day, by=list(dlw$Species), FUN="mean", na.omit=T)
dlw_something <- aggregate(dlw$Initial_mass, by=list(dlw$Species), FUN="mean", na.omit=T)
dlw_mean <- merge(dlw_mean, dlw_something, by = "Group.1")
names(dlw_mean) <- c("Species", "kJ_day", "Mass_g")

ggplot(dlw_mean, aes(log(Mass_g), log(kJ_day))) + geom_point(aes(col=Species), size=4) + theme_bw() + geom_smooth(method=lm)
...
fit3 <- lm(log(kJ_day) ~ log(Mass_g),data = dlw_mean)
summary(fit3)
```