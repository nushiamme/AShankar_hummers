...
dlw_sp_mean_log <- ggplot(dlw_mean, aes(log(Mass_g), log(kJ_day))) + 
  geom_point(aes(col=Species), size=6) + geom_smooth(method=lm)+ my_theme + 
  theme(legend.key.height = unit(1, "line")) + 
  xlab("log(Mass)") + ylab("log(Energy expenditure (kJ/day))") +
  geom_text(x = 1.5, y = 4.5, label = lm_eqn(log(dlw_mean$kJ_day), log(dlw_mean$Mass_g)), parse=T)
plot(dlw_sp_mean_log, fig.align='left')
```

A plot of daily energy expenditure (kJ) vs. Mass (g), including literature and study values, non-log
```{r, echo=F, fig.cap=""}
dlw_sp_mean <- ggplot(dlw_mean, aes(Mass_g, kJ_day)) + 
  geom_point(aes(col=Species), size=4) + theme_bw() + geom_smooth(method=lm) +
  geom_text(x = 5, y = 4.5, label = lm_eqn(dlw_mean$kJ_day, dlw_mean$Mass_g), parse=T) +
  theme(legend.key.height = unit(1, "line"))
...
```{r, echo=F}
dee_per_gram <- ggplot(subset(dlw,Reasonable_not %in% "Y"), aes(Initial_mass, kJ_dayg)) + 
...
dee_total <- ggplot(subset(dlw,Reasonable_not %in% "Y"), aes(Initial_mass, kJ_day)) + 
  ...
  ylab("kJ per day") + xlab("Initial mass (g)")
...
dee_indiv_log <- ggplot(dlw_mean, aes(log(Mass_g), log(kJ_day))) + 
  geom_point(aes(shape=Region), size=5) + theme_bw() + xlab("Log(Mass (g))") +
...
dee_indiv_EC <- ggplot(subset(dlw,Big_site %in% "EC"), aes(log(Initial_mass), log(kJ_day))) +
  geom_point(aes(shape=Species), size=5) + theme__bw() + xlab("Log(Mass (g))") + 
  ylab("Log(kJ per day)")  + geom_smooth(method=lm) + 
  theme(strip.background = element_blank(), 
        panel.border = element_rect(colour = "black", fill=NA), legend.key.height=unit(2,"line"))
dee_indiv_EC
...
dee_indiv_AZ <- ggplot(subset(dlw,Big_site %in% "AZ"), aes(log(Initial_mass), log(kJ_day))) +
  geom_point(aes(shape=Species), size=5) + theme_bw() + xlab("Log(Mass (g))") + 
  ylab("Log(kJ per day)")  + geom_smooth(method=lm) + 
  theme(strip.background = element_blank(), 
        panel.border = element_rect(colour = "black", fill=NA), legend.key.height=unit(2,"line"))
dee_indiv_AZ
