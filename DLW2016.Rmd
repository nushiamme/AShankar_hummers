...
setwd("C:\\Users\\shankar\\Dropbox\\DLW_paper")
...
dlw_sp_mean_log <- ggplot(dlw_mean, aes(log(Mass_g), log(kJ_day))) + 
  geom_point(aes(col=Species), size=6) + my_theme + geom_smooth(method=lm)+
  theme(legend.key.height = unit(3, "line")) + xlab("log(Mass)") + ylab("log(Energy expenditure (kJ/day))") +
  geom_text(x = 1.5, y = 4.5, label = lm_eqn(log(dlw_mean$kJ_day), log(dlw_mean$Mass_g)), parse=T, size=10)
