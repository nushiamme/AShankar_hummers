...
dee_total_log <- ggplot(subset(dlw,Reasonable_not %in% "Y"), aes(log(Initial_mass), log(kJ_day))) + geom_point(aes(color=Species), size=2) + theme_classic(base_size=30) + xlab("Log(Initial mass (g))") + scale_color_brewer(palette="Paired") + ylab("Log(kJ per day)")  + geom_smooth(method=lm) +
