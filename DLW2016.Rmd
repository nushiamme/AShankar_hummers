...
dlw_sp_mean <- ggplot(dlw_mean, aes(Mass_g, kJ_day, label=Species)) + 
  ...
  geom_text(x = 5, y = 75, label = lm_eqn(dlw_mean$kJ_day, dlw_mean$Mass_g), parse=T) +
  geom_text(hjust = 0, nudge_x = 0.03) +
  scale_colour_manual(values = getPalette(colourCount)) +
...
A plot of daily energy expenditure (kJ) vs. Mass (g), with species' mean in circles and individuals measurements in traingles:
```{r, echo=F, fig.align="center"}
dlw_indiv_and_sp_mean <- ggplot(NULL, aes(Mass_g, kJ_day)) +
  geom_point(data=dlw, aes(Mass_g, kJ_day, fill=Species), size=2, shape=24, alpha=0.7) +
  geom_point(data=dlw_mean, aes(Mass_g, kJ_day, fill=Species), size=4, shape=21) + my_theme + 
  scale_colour_manual(values = getPalette(colourCount)) +
  scale_fill_manual(values = getPalette(colourCount)) +
  theme(legend.key.height = unit(1, "line")) + xlab("Initial mass (g)") + ylab("Daily Energy Expenditure (kJ/day)")
plot(dlw_indiv_and_sp_mean)
```


Daily energy expenditure vs. Mass, colored by species:
```{r, echo=F, fig.align="center"}
dee_total <- ggplot(dlw, aes(Mass_g, kJ_day)) + 
  geom_point(aes(col=Species), size=2, shape=16, alpha=0.8) + theme_bw() + 
  scale_colour_manual(values = getPalette(colourCount)) +
