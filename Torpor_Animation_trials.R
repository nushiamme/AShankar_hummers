...
for (tslot in unique(gcb_0720$TimeSlot)){
  ...
  gcb_gif_temp <- ggplot(p_temp, aes(SampleNo, Time2, frame = Time_chunks, col=Category)) +
  ...
  gganimate(gcb_gif_temp, paste("gcb0720_trial3_",tslot,".gif"), interval=0.05)
}

gcb_gif_1 <- ggplot(gcb_0720[gcb_0720$TimeSlot==c(5,6),], aes(SampleNo, EE_J, frame = BirdID, col=Category)) +
...
gcb_gif_6 <- ggplot(gcb_0720[gcb_0720$TimeSlot==6,], aes(Time2, EE_J, frame = Time_chunks, col=Category)) +
  geom_path(aes(cumulative=T)) + my_theme_blank + theme(axis.text.x = element_text(angle=60, hjust=1, size=20),
                                                        axis.line.x = element_line(colour = "grey50"),
                                                        axis.line.y = element_line(colour = "grey50")) +
  scale_color_manual(values = "red") + xlab("Time") #+ xlim(0,4060)

# Make the animation!
gganimate(gcb_gif_6, interval=0.075)
...
gcb_gif_total <- ggplot(gcb_0720, aes(SampleNo, EE_J, frame = BirdID, col=Category)) +
  ...
  theme(axis.text.x = element_text(angle=60, hjust=1, size=20),
        ...
        axis.line.y = element_line(colour = "grey50")) +
  ylim(0,50) + xlab("Time (seconds)") + ylab("Energy expenditure (J)")
gganimate(gcb_gif_total, "gcb_0720_total.gif", interval=0.05)
