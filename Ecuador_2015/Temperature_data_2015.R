...
Temp_plot <- ggplot(Ta_JulAug, aes(Time,Temperature)) + theme_bw() + geom_point(aes(col=as.factor(Day))) + 
  scale_color_discrete() + geom_smooth(aes(group=1)) + facet_grid(iButton_number~Month) + 
